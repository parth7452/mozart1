 ...
 *
 * This is an operator's job, not a request path. It connects as the app does
 * (ADR 0034): the org and the member it acts as come from
 * `app.member_for_link()` as `app_rw`, and every read and write after that goes
 * through `PostgresStore` as `app_rw` with that member's claims, so the tenant
 * policies apply to the change the same way they apply to the app. `DATABASE_URL`
 * is the same login the app uses, never the owner.
 */
...
async function main(): Promise<void> {
  ...
  const store = new PostgresStore(config, { orgId: actor.orgId, userId: actor.userId });

  try {
    if (!backfillOnly) {
      const debtor = await store.debtorByRetailerKey(retailerKey as string);
      if (debtor === undefined) {
        const known = await store.retailerKeys();
        throw new Error(
          `no debtor with retailer_key ${JSON.stringify(retailerKey)} in ${slug}. ` +
            ...
            `Known keys: ${known.join(', ') || '(none)'}`,
        );
      }
      if (dryRun) {
        console.log(
          `would add alias ${JSON.stringify(alias)} → ${debtor.displayName}`,
        );
      } else {
        await store.addDebtorAlias(debtor.debtorId, alias as string);
        console.log(`alias ${JSON.stringify(alias)} → ${debtor.displayName}`);
      }
    }
    ...
    if (dryRun) {
      ...
      const pending = await store.countUnmatchedCases();
      console.log(`${pending} unmatched case(s) would be re-checked`);
      return;
    }
