...
- **Dense documents:**
  - a remittance past about 120 rows is read again in two-page parts and
    ...
    $1.05, but took 344 seconds, past the job's 300-second limit, so in
    production a remittance that dense is expected to fail after retries
    until the founder picks a fix (ADR 0053, *Consequences*); until then,
