...
| F5 | **After a merge where the newer copy survives, a third copy matches nothing.** The probable matcher reads the invoice number off the case's own identifier rows. The survivor's own row was skipped as a collision when it arrived. | `store.ts` `knownOpenDeductions`, `discovery.ts` `knownDeductions` | A third copy opens with no pair, so it counts again. | **Fixed.** Both readers take the invoice number over the case and every case merged into it, through `deduction_merges_current`, as `identityCandidates` already did, and prefer the case's own row, so a merged-away half that printed another invoice never displaces the survivor's. The in-memory doubles model no merges and say so. Tests: `store-postgres/test/duplicate-merge.test.ts` (the third copy is a probable match for the survivor on `openCase`, and `knownDeductions` names the survivor with the invoice; with halves on different invoices, the survivor keeps its own). | No migration |
...
1. **F1** (done, pilot E7) **and F5** (done), and the case-list figures: code only, no migration.
