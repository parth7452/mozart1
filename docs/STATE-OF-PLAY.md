...
| **Closed sign-ups, and the claims guard** (ADR 0045, migration 0033) | 0033 was applied to `mozart-preview` and then production on 2026-09-24 and read back on both (md5, both functions still definer and pinned, same results and grants, a `sub`-only caller refused). The web half deployed on merge. What would prove it: both members sign in through the new form; an address with no auth user gets the same "sent" page and no mail, with `otp_disabled` in the log; a person invited from the dashboard follows the invitation once and then signs in from the form; then the founder switches off "Allow new users to sign up". **2026-09-26: those last two steps are superseded.** The founder switched sign-ups **on**, gated by ADR 0051 §6: the form asks the database (`app.address_is_invited()`) and lets the provider create an account only for an invited address; a before-user-created hook (`hooks.before_user_created`), which the founder enables in the dashboard once migration 0035 is applied, refuses an account for any address nobody invited, however it was asked for; and `requireSession` refuses a session not made by an email link, a password sign-in included. A person an owner adds on Settings → Team needs no dashboard invitation: their first link makes the account. 0035 is not yet applied anywhere, `mozart-preview` first |
