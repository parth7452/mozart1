...
- **Failure alerts** (ADR 0052), 2026-09-26. `ALERT_EMAIL_TO`,
  `ALERT_EMAIL_FROM` and `RESEND_API_KEY` went onto Vercel Production at 07:12
  UTC, and the redeploy registered with Inngest at 07:13. A
  `recouple/alert.test` event sent from the Inngest dashboard delivered
  "[TEST] Mozart: failure alerts are working" from `alerts@mozart.financial`
  at 07:15 (VERIFY-CHECKLIST §10.2). No real failure has been emailed yet, so
  §10.3's check of a real alert is still to come
